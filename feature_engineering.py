...
def build_two_vector_features(
    ...
    for _, row in df.iterrows():
        ...
        # ============================
        # ENGINEERED DIFFERENCE FEATURES (from Model 1)
        # ============================

        # 1. Core difference features
        p1['rank_diff']         = row.get('winner_rank', np.nan)        - row.get('loser_rank', np.nan)
        p1['rank_points_diff']  = row.get('winner_rank_points', np.nan) - row.get('loser_rank_points', np.nan)
        p1['age_diff']          = row.get('winner_age', np.nan)         - row.get('loser_age', np.nan)
        p1['height_diff']       = row.get('winner_ht', np.nan)          - row.get('loser_ht', np.nan)

        # 2. Seed features
        p1['seed_diff']    = row.get('winner_seed', np.nan) - row.get('loser_seed', np.nan)
        p1['p1_is_seeded'] = int(not pd.isna(row.get('winner_seed')) and row.get('winner_seed', 0) > 0)
        p2['p2_is_seeded'] = int(not pd.isna(row.get('loser_seed'))  and row.get('loser_seed', 0) > 0)

        # 3. Experience proxy (age)
        p1['p1_experience']   = row.get('winner_age', np.nan)
        p2['p2_experience']   = row.get('loser_age', np.nan)
        p1['experience_diff'] = p1['p1_experience'] - p2['p2_experience']
        ...
        if include_swapped:
            ...
            # Flip difference features so they reflect p1's perspective
            swapped = {**sw1, **sw2, **m, 'target': 0}
            swapped['rank_diff']        = -p1['rank_diff']
            swapped['rank_points_diff'] = -p1['rank_points_diff']
            swapped['age_diff']         = -p1['age_diff']
            swapped['height_diff']      = -p1['height_diff']
            swapped['seed_diff']        = -p1['seed_diff']
            swapped['experience_diff']  = -p1['experience_diff']
            rows.append(swapped)
